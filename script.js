...
function setupLazyMediaLoading() {
  if (!("IntersectionObserver" in window)) {
    ...
    document
      .querySelectorAll("video[data-src], img[data-src]")
      .forEach(loadMediaElement);
    return;
  }
...
function setupHeroLogoHover() {
  ...
  const enterHandler = () => {
    ...
    if (designerTitle) {
      ...
      setTimeout(() => {
        designerTitle.style.transition =
...
function setupSmoothScrolling() {
  ...
  navLinks.forEach((link) => {
    link.addEventListener("click", function (e) {
      ...
      // Only handle links that start with # and are on the current page
      if (!href || !href.startsWith("#")) {
        return; // Let normal navigation happen for external links
      }
      ...
      // Special handling for "Home" link - always scroll to top
      if (targetId === "hero" || targetId === "home" || href === "#") {
        window.scrollTo({
          top: 0,
          behavior: "smooth",
        });

        // Update URL without hash, preserving history
      ...
      if (targetElement) {
        ...
        // Update URL without adding hash to history
        if (history.pushState) {
          history.pushState(
            null,
            null,
  ...
  // Handle back button to restore scroll position
  window.addEventListener("popstate", function () {
    // Browser automatically restores scroll position
    // Just update active nav based on current scroll position
    updateActiveNavigationNow();
  });

  // Handle navbar logo click - always go to top
  const navbarLogos = document.querySelectorAll(".navbar-logo, .navbar-left a");
...
function updateActiveNavigation() {
  ...
  function updateActiveNavigationNow() {
    let current = "";
    ...
    navLinks.forEach((link) => {
      ...
      // Check if at top of page for home link
      if (
  ...
  // Make updateActiveNavigationNow available globally for popstate
  window.updateActiveNavigationNow = updateActiveNavigationNow;
}
